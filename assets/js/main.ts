...
class CounterApp {
    ...
    lapData: Lap[] = [];
    overallTime: [number, number, number, number] = [0, 0, 0, 0]; // hour, minute, seconds, cs
    ...
    private incrementTime(time: [number, number, number, number], centiToAdd: number): [number, number, number, number] {
        let [h, m, s, cs] = time;
        cs += centiToAdd;
